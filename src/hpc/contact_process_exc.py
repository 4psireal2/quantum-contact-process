...
def main():
    ...
    evp_residual_exc = (lindblad_hermitian @ eigentensors[1] - eigenvalues[1] * eigentensors[1]).norm()**2
    logger.info(f"Eigensolver error for first excited state: {evp_residual_exc}")

    evp_residual_ground = (lindblad_hermitian @ eigentensors[0] - eigenvalues[0] * eigentensors[0]).norm()**2
    logger.info(f"Eigensolver error for ground state: {evp_residual_ground}")
    np.savetxt(OUTPUT_PATH + f"evp_residual_L_{L}_D_{bond_dim}_O_{OMEGA}_{SLURM_ARRAY_JOB_ID}.txt",
               np.array([evp_residual_ground, evp_residual_exc]),
               fmt='%.6f')
    ...
    np.savetxt(OUTPUT_PATH + f"non_hermit_exc_L_{L}_D_{bond_dim}_O_{OMEGA}_{SLURM_ARRAY_JOB_ID}.txt",
    ...
    np.savetxt(OUTPUT_PATH + f"n_s_mps_exc_L_{L}_D_{bond_dim}_O_{OMEGA}_{SLURM_ARRAY_JOB_ID}.txt",
               np.array([n_s]),
               fmt='%.6f')
    ...
    n_s = np.mean(particle_nums)
    logger.info(f"Mean Particle number: {n_s}")
    np.savetxt(OUTPUT_PATH + f"n_s_mpo_exc_L_{L}_D_{bond_dim}_O_{OMEGA}_{SLURM_ARRAY_JOB_ID}.txt",
               np.array([n_s]),
               fmt='%.6f')
    ...
    np.savetxt(OUTPUT_PATH + f"darkst_overlap_exc_L_{L}_D_{bond_dim}_O_{OMEGA}_{SLURM_ARRAY_JOB_ID}.txt",
               np.array([overlap]),
               fmt='%.6f')
